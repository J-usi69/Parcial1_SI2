...
@permiso_controller.get("/", response_model=list[PermisoResponse])
def get_permisos(db: Session = Depends(get_db), svc: PermisoService = Depends(get_pm_service)):
    return svc.get_permisos(db)

@permiso_controller.post("/", response_model=PermisoResponse)
...
@permiso_controller.patch("/{permiso_id}", response_model=PermisoResponse)
...
@permiso_controller.delete("/{permiso_id}")
